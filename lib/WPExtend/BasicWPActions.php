...
/**
 * Autoloder for WordPress Action Hooks. Supports almost 500 WordPress action hooks at the moment.
...
final class BasicWPActions {
	...
	public static function GetHtmlTable() {
		...
		foreach ( self::$_actions as $key => $value ) {
			$html [ ] = '<tr><td><a href="http://codex.wordpress.org/Plugin_API/Action_Reference/' . $key . '" target="_blank" style="text-decoration: none;">' . $key . '</a></td><td>' . $value . '</td></tr>';
		}
		$html [ ] = '</table>';
	...
	private static $_actions = array(
			...
			'akismet_tabs' => 'OnActionAkismetTabs',
			...
			'create_category' => 'OnActionCreateCategory',
			'create_term' => 'OnActionCreateTerm',
			'created_category' => 'OnActionCreatedCategory',
			...
			'delete_category' => 'OnActionDeleteCategory',
			...
			'edit_category' => 'OnActionEditCategory',
			...
			'edited_category' => 'OnActionEditedCategory',
			...
			'publish_page' => 'OnActionPublishPage',
			'publish_phone' => 'OnActionPublishPhone',
			'publish_post' => 'OnActionPublishPost',
